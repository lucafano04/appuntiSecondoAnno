...
\section{Trasformazioni Cicliche}
    ...
    \subsection{Ciclo di Carnot}
        Il ciclo di Carnot è un ciclo termodinamico ideale in quattro fasi, può essere rappresentato graficamente come segue:
        \begin{figure}[H]
            \centering
            \begin{tikzpicture}
                \begin{axis}[
                    ...
                    % Adiabatiche
                    \addplot[domain=0.3:3, thick,samples=100, red] {1/(x-(1/5))^2};
                    \addplot[domain=0.3:3, thick,samples=100, green] {2/(x-(1/5))^2};
                    % Isoterme
                    \addplot[domain=0.15:3, thick,samples=100, blue] {1/x};
                    \addplot[domain=0.15:3, thick,samples=100, yellow] {2/x};
                    % Punti 
                    \addplot[only marks, mark=*, mark options={scale=1}, color=red] coordinates {(0.85311, 2.34436)};
                    \node at (axis cs:0.85311, 2.34436) [anchor=west] {$A$};
                    \addplot[only marks, mark=*, mark options={scale=1}, color=red] coordinates {(1.37082, 1.45898)};
                    \node at (axis cs:1.37082, 1.45898) [anchor=west] {$B$};
                    \addplot[only marks, mark=*, mark options={scale=1}, color=red] coordinates {(2.38322, 0.4196)};
                    \node at (axis cs:2.38322, 0.4196) [anchor=west] {$C$};
                    \addplot[only marks, mark=*, mark options={scale=1}, color=red] coordinates {(1.37082, 0.72949)};
                    \node at (axis cs:1.37082, 0.72949) [anchor=west] {$D$};
                \end{axis}
            \end{tikzpicture}
        \end{figure}
        questo ciclo è composto da due trasformazioni adiabatiche e due trasformazioni isoterme. Le trasformazioni isoterme sono rappresentate dalle curve blu e gialla, mentre le trasformazioni adiabatiche sono rappresentate dalle curve rosse e verdi. \newline
        In questo ciclo il lavoro è dato dall'area racchiusa tra le curve, per calcolarne il valore dobbiamo calcolare il lavoro svolto dalle diverse trasformazioni, il quale sarà dato da:
        \begin{align*}
    ...
    \subsection{Ciclo di Otto - Motori Diesel} 
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}
                ...
                % Curve isoterme
                \addplot[domain=0.7:4.2, thick, samples=100] {3/x}; % curva superiore: 3 -> 4
                \addplot[domain=0.7:4.2, thick, samples=100] {2/x}; % curva inferiore: 2 -> 1

                % Segmenti verticali e orizzontali
                \addplot[thick] coordinates {(0.7,2.85714) (0.7,4.28571)}; % verticale: 2 -> 3
                \addplot[thick] coordinates {(4.2,0.47619) (4.2,0.71429)}; % verticale: 4 -> 1

                \addplot[thick] coordinates {(1,0.47619) (4.2,0.47619)}; % orizzontale: 0 -> 1    
                
                \addplot[only marks, mark=*, mark options={scale=1}, color=red] coordinates {(4.2,0.71429)};
                \addplot[only marks, mark=*, mark options={scale=1}, color=red] coordinates {(0.7,2.85714)};
                \addplot[only marks, mark=*, mark options={scale=1}, color=red] coordinates {(0.7,4.28571)};
                \addplot[only marks, mark=*, mark options={scale=1}, color=red] coordinates {(4.2,0.47619)};
                \addplot[only marks, mark=*, mark options={scale=1}, color=red] coordinates {(1,0.47619)};

                % Etichette
                \node at (axis cs:4.2,0.71429) [anchor=north west] {$A$};
                \node at (axis cs:0.7,2.85714) [anchor=north east] {$B$};
                \node at (axis cs:0.7,4.28571) [anchor=south east] {$C$};
                \node at (axis cs:4.2,0.47619) [anchor=south west] {$D$};
                \node at (axis cs:1,0.47619) [anchor=south west] {$O$};

                \end{axis}
            \end{tikzpicture}
        \end{figure}
        Nella prima fase del ciclo si osserva una espansione isobara, nella quale il pistone si espande premettendo l'ingresso di una miscela di aria e benzina nel cilindro, il tutto a pressione constante (valvola aperta). Poi si ha una compressione adiabatiche, in cui il pistone si muove verso l'alto comprimendo la miscela di aria e benzina (valvola chiusa, sistema isolato). Poi avviene la fase di scoppio della miscela di aria e benzina, in cui la miscela viene accesa da una scintilla e quindi si verifica un improvviso ed irreversibile aumento di pressione e temperatura (valvola chiusa), questo fenomeno porta negli instanti successivi ad una espansione del volume del gas, il quale compie lavoro sul pistone (valvola chiusa). Quando il gas si trova al massimo volume viene espulso aprendo la valvola di scarico (valvola aperta) e diminuendo irreversibilmente la pressione. Viene infine chiusa la valvola e compresso il pistone fino a tornare alla posizione iniziale (valvola chiusa). \newline
